...
function addProduto($conn, $nome_produto, $quantidade_min, $quantidade_atual) {
    $sql = "INSERT INTO estoque (nome_produto, quantidade_min, quantidade_atual) VALUES (:nome, :min, :atual)";
    $stmt = $conn->prepare($sql);
    $stmt->bindparam(':nome', $nome_produto);
    $stmt->bindparam(':min', $quantidade_min);
    $stmt->bindparam(':atual', $quantidade_atual);
    $stmt->execute();
}

function updateProduto($conn, $id, $nome_produto, $quantidade_min, $quantidade_atual) {
   $sql = "UPDATE estoque SET nome_produto = :nome, quantidade_min = :min, quantidade_atual = :atual WHERE id = :id";
   $stmt = $conn->prepare($sql);
   $stmt->bindparam(':nome', $nome_produto);
   $stmt->bindparam(':min', $quantidade_min);
   $stmt->bindparam(':atual', $quantidade_atual);
